...
trouble-shootings:
	...
	* As a last resort, go ahead and create a new Security Group with `All traffic` and `0.0.0.0/0` and attach it to your `vm1` and `vm2` instance and remove the old Security Group from `vm1` and `vm2`. 
...
* **Spark Job Hanging:** If your PageRank code runs forever and won't finish, the most likely culprit is that you have a previously usbmitted Spark job not stopped yet. Use Spark Dash to monitor any hanging job. If any, kill it or stop it using `spark.stop()`. This would occur mostly likely when you use an interactive Notebook session: when you do interactive debugging, you forget to hit the cell that contains the `spark.stop()` statement; and that submitted job won't be automatically removed and that blocks your next job. 
