...
```sh
% scp hadoop-3.3.4/etc/hadoop/core-site.xml hadoop-3.3.4/etc/hadoop/hdfs-site.xml hadoop-3.3.4/etc/hadoop/workers ubuntu@<IP_of_vm2>:~/hadoop-3.3.4/etc/hadoop/
```
...
breakdown of the execution statistics, etc.  A caveat of this with
EC2 is that you will need to manually replace the private IPv4
address with the public IPv4 DNS address of `vm1` in order to view
the application detail. Otherwise, you will see a `We can’t connect
to the server at ...` error.
...
neighbors. You need to decompress the downloaded `.gz` dataset (using `gunzip`) and
copy the input file to HDFS first. 
...
convension of `LastName_FirstName_ComputingID_A1.tar.gz`. The
...
Put the code of each part and each task into separate folders and
givn them meaningful names. Code should be commented well (which will
be worth some percentage of your grade for the assignment, the GTA
will be looking at your code). In addition, put a `README` file for
each task and provide instructions about how to run your code. 

For the **report**, you should report the application completion time
with respect to Task 1-4. Present or reason about the difference in
performance or your own findings, if any. Take a look at the lineage
graphs of applications from the Spark UI, or investigate into the log
to find the amount of network/storage read/write bandwidth and the
number of tasks for every execution may help you better understand
