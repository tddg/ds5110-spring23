...
[PageRank](http://infolab.stanford.edu/~backrub/google.html) algorithm 
([the Wikipedia version](https://en.wikipedia.org/wiki/PageRank#Simplified_algorithm)),
...
changed in several ways. For example, any shuffle operation on a
...
changes.
* **Task 3.** Persist the appropriate RDDs as in-memory
...
of the Spark Worker on `vm2` and then use `kill -9 <Worker_PID>` to kill
...
with respect to Task 1-4. Present or reason about the differences in
performance or your own findings, if any. Take a look at the DAG
lineage graphs of applications from the `Application Detail UI`, or
investigate into the log to find the amount of network/storage
read/write bandwidth and the number of tasks for every execution may
help you better understand the performance issues.
